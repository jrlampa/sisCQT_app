...
def gerar_diagrama(
    ...
    for _, row in df.iterrows():
        ...
        # Destaque do Baricentro (Prioridade Visual)
        # Normaliza strings para garantir match ('5' == '5')
        p_str = str(ponto).strip().upper()
        b_str = str(ponto_baricentro).strip().upper() if ponto_baricentro else ""

        if p_str == b_str and b_str != "":
            fillcolor = "#E1BEE7"  # Roxo claro
            color = "#8E24AA"  # Roxo escuro borda
            penwidth = "3.0"
