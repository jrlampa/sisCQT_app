...
class ElectricalEngine:
    ...
    def limpar_linhas_vazias(df: pd.DataFrame) -> pd.DataFrame:
        ...
        def tem_dados(row):
            ...
            try:
                mts = float(row.get("METROS", 0))
            except:
                mts = 0.0
            try:
                ip_qtd = int(row.get("QTD_IP", 0))
            except:
                ip_qtd = 0
            carga = 0
            for c in ["MONO", "BIFÁSICO", "TRIFÁSICO", "TRI ESPECIAL", "CARGA_ESP_KVA"]:
                try:
                    carga += float(row.get(c, 0))
                except:
                    pass
    ...
    def sanitizar(
        ...
        # 1. Garantir Colunas Obrigatórias (Preencher com padrão se faltar)
        cols_default = {
            "PONTO": "",
            "MONTANTE": "",
            "CABO": "",
        ...
        # 3. Tratamento Numérico (Correção de Vírgula e Conversão)
        cols_num = [
            "METROS",
            "MONO",
            "BIFÁSICO",
        ...
        for c in cols_num:
            # Converte para string primeiro para replace seguro
            ...
            if (df[c] < 0).any():
                erros.append(f"Valores negativos detectados e mantidos em {c}.")
    ...
    def sugerir_centro_carga(df: pd.DataFrame) -> str:
        ...
        try:
            ...
            if col_accum in df_net.columns:
                ...
                return df_net.sort_values(by=col_accum, ascending=False).iloc[0][
                    "PONTO"
                ]
            return "TRAFO"
        except:
    ...
    def balancear_fases(df: pd.DataFrame, ordem: List[str], pmap: Dict):
        ...
        for n in ordem:
            ...
            for _ in range(int(m)):
                fl = min(fases, key=fases.get)
                fases[fl] += 1
                sugestao.append(f"1M->{fl}")
            for _ in range(int(b)):
                ...
                fases[pl[0]] += 1
                fases[pl[1]] += 1
                sugestao.append(f"1B->{pl[0]}{pl[1]}")
            df.at[i, "SUGESTAO_BALANCEAMENTO"] = (
                ", ".join(sugestao) if sugestao else "-"
            )
    ...
    def calcular(
        ...
        coef_cabos = config_context["cabos"]
        mapa_ips = config_context["ips"]
        perfis = config_context["perfis"]
        nome_perfil = params.get("perfil", "Padrão (Urbano)")
        limites = perfis.get(
            nome_perfil,
            {
                "cqt_max": 6.0,
                "sobrecarga_max": 100.0,
                "metros_max": 3000.0,
                "clientes_max": 2000,
            },
        )
        ...
        df = df.reset_index(
            drop=True
        )  # Reseta índice para garantir alinhamento posicional (0, 1, 2...)
        ...
        for n in ordem:
            ...
            if cabo not in coef_cabos and df.at[i, "METROS"] > 0:
                avisos_diagnostico.append(
                    f"ALERTA: Ponto '{n}' tem cabo sem coeficiente."
                )
